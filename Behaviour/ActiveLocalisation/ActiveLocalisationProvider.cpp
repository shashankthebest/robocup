...
#include "GenerateEpisodeState.h"
#include "EvaluateEpisodeState.h"
#include "PauseEpisodeState.h"

#include "LocaliseSelfState.h"
#include "LocaliseBallState.h"
#include "TryLineUpState.h"
#include "TryKickState.h"
...
ActiveLocalisationProvider::ActiveLocalisationProvider(Behaviour* manager) : BehaviourFSMProvider(manager)
{
    #if DEBUG_BEHAVIOUR_VERBOSITY > 1
        debug << "ActiveLocalisationProvider::ActiveLocalisationProvider" << endl;
    #endif
    initOptimiser();
 ...
 //   else
  //      m_log.open((DATA_DIR + "/Optimisation/" + m_id + "Log.log").c_str(), fstream::out | fstream::app);
    ...
    m_generate     = new GenerateEpisodeState(this);
    m_evaluate     = new EvaluateEpisodeState(this);

    m_pause        = new PauseEpisodeState(this);
...
ActiveLocalisationProvider::~ActiveLocalisationProvider()
{
    delete m_generate;
    delete m_evaluate;
    delete m_pause;
    delete m_localiseSelf;
...
BehaviourState* ActiveLocalisationProvider::nextStateCommons()
{
    while (Blackboard->GameInfo->getCurrentState() != GameInformation::PlayingState)
		...
		if (m_state == m_pause and Platform->getTime() > 5000)
			{
			    cout<<"\nreturned generate\n";
			    return m_generate;
			}
...
void ActiveLocalisationProvider::initOptimiser()
{
    /*
 ...
 */
float ActiveLocalisationProvider::normalDistribution(float mean, float sigma)
{
